...
func (t *TypedefType) sizeIntl(recCheck recCheck) int64 {
	...
	defer release()
	if t.Type == nil {
		fmt.Printf("t.Type is nil %T(%p) name:%q\n", t, t, t.Name)
	}
	return t.Type.sizeIntl(recCheck)
}
...
func readType(d *dwarf.Data, name string, r *dwarf.Reader, off dwarf.Offset, typeCache map[dwarf.Offset]Type) (Type, error) {
	...
	if t, ok := typeCache[off]; ok {
		switch uint32(off) {
			case 0x4f2db, 0x4f1e2, 0x4ff21:
			fmt.Printf("DEBUG typecache hit  %x\n", off)
		}
		return t, nil
	}
	switch uint32(off) {
		case 0x4f2db, 0x4f1e2, 0x4ff21:
			fmt.Printf("DEBUG typecache miss %x\n", off)
	}
	r.Seek(off)
	e, err := r.Next()
	if err != nil {
	...
	switch uint32(off) {
		case 0x4f2db, 0x4f1e2, 0x4ff21:
		fmt.Printf("DEBUG seek %x -> %q %s\n", off, e.Val(dwarf.AttrName).(string), e.Tag)
	}
	...
	typeOf := func(e *dwarf.Entry, attr dwarf.Attr) Type {
		...
		case dwarf.Offset:
			switch toff {
				case 0x4f2db, 0x4ff21, 0x4f1e2:
				fmt.Printf("DEBUG typeOf(entry=%8x attr=%s) e.Val=%x\n", e.Offset, attr, toff)
				default:
				fmt.Printf("DEBUG typeOf(entry=%8x, attr=%s) e.Val=%x, entry.Name=%q\n", e.Offset, attr, toff, e.Val(dwarf.AttrName))
			}
	...
	case dwarf.TagArrayType:
		...
		if off == 0x4ff21 {
			fmt.Printf("DEBUG [%8x] arrayType.Type=%T(%p, off=%x) arrayType=%q\n", e.Offset, t.Type, t.Type, e.Val(dwarf.AttrType), t.Name)
		}
	...
	case dwarf.TagTypedef:
		...
		t.ReflectKind = getKind(e)
		if off == 0x4f2db {
			fmt.Printf("DEBUG typedef [%8x] %p RefrectKind:%v\n", off, t, t.ReflectKind)
		}
		switch t.ReflectKind {
		case reflect.Map:
			m := new(MapType)
		...
		t.Name, _ = e.Val(dwarf.AttrName).(string)
		if off == 0x4f2db {
			fmt.Printf("DEBUG typedef [%8x] %p Name:%q, e.AttrType:%x(%T)\n", off, t, t.Name, e.Val(dwarf.AttrType), e.Val(dwarf.AttrType))
		}
		t.Type = typeOf(e, dwarf.AttrType)
	...
	switch uint32(off) {
		case 0x4f2db, 0x4f1e2, 0x4ff21:
		fmt.Printf("DEBUG readType(%8x) %s %q t:%p, err:%s\n", off, e.Tag, e.Val(dwarf.AttrName), typ, err)
	}
	if err != nil {
		goto Error
	}
